...
public:
	void load() override {
		...
		GameObject* red_stamp = scene_root->addChild(pref->InstantiatePrefab("P_Stamp", 688, 412));
		GameObject* stamp_tray = scene_root->addChild(pref->InstantiatePrefab("P_Renderable_Object", 668, 252));
		...
		stamp_tray->getTransform()->setLocalScale(4, 4);
	}
};